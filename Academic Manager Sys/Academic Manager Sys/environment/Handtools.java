...
public class Handtools {
	...
	public static final ArrayList<Project> locateProjectWithItParticipant(List<Project> list, User user){
		ArrayList<Project> matchs = new ArrayList<Project>();
		Project target = null;
		...
		for(int i = 0; i < list.size(); ++i){
			target = list.get(i);
			if(target != null){
				participants = target.getParticipants();
				for(int j = 0; j < participants.size(); ++j){
					if(participants.get(j) == user){
						matchs.add(target);
						break;
					}
				}
	...
	public static final ArrayList<Project> locateProjectByTitle(List<Project> list, String title){
		ArrayList<Project> matchs = new ArrayList<Project>();
		Project target = null;
	...
	public static final ArrayList<Issue> locateIssuePerProjectRelated(List<Issue> list, Project related){
		ArrayList<Issue> matchs = new ArrayList<Issue>();
		
		for(int i = 0; i < list.size(); ++i){
			if(list.get(i).getRelated() == related){
				matchs.add(list.get(i));
			}
		}
	...
	public static final int[] countCollaboratorByType(List<User> list){
		int count[] = {0, 0, 0, 0, 0, 0}; // index: 0 - Administrator, 1 - Graduating, 2 - Master,
														// 3 - PhD, 4 - Teacher, 5 - Total;
		for(int i = 0; i <= list.size(); ++i){
			if(i == list.size()) break;
				...
				if(list.get(i) != null){
					...
					else if(list.get(i) instanceof Graduating){
						count[1]++;
					}
					else if(list.get(i) instanceof Master){
						count[2]++;
					}
					else if(list.get(i) instanceof PhD){
						count[3]++;
					}
	...
	public static final int[] countProjectsByState(List<Project> list){
		int count[] = {0, 0, 0, 0}; // index: 0 - in draft, 1 - in progress, 2 - accomplished, 3 - total
		for(int i = 0; i <= list.size(); ++i){
			if(i == list.size()) break;
			else{
				if(list.get(i) != null){
	...
	public static final int[] countIssuePerType(List<Issue> list){
		int count[] = {0, 0, 0}; // 0 - Articles, 1 - Guidance, 2 - Total
		for(int i = 0; i <= list.size(); ++i){
			if(i == list.size()) break;
			else{
				if(list.get(i) != null){
